...
try {
	...
	await page.screenshot({ path: resolve(screenshotDir, '01-home-signed-out.png'), fullPage: true });
	...
	const headerSignIn = page.locator('header button', { hasText: /^Sign in$/i }).first();
	const headerSignUp = page.locator('header button', { hasText: /Sign up|Join/i }).first();
	...
	check('Home: clicking Sign in did NOT navigate', urlBefore === urlAfter, `before=${urlBefore} after=${urlAfter}`);
	...
	check('Home: Clerk modal is visible after Sign in click', modalCount > 0, `modalElements=${modalCount}`);
	await page.screenshot({ path: resolve(screenshotDir, '02-home-after-signin-click.png'), fullPage: true });
	...
	await page.screenshot({ path: resolve(screenshotDir, '03-mcp-signed-out.png'), fullPage: true });
	...
	check('/mcp: Clerk modal is visible after Get access click', modalCountMcp > 0, `modalElements=${modalCountMcp}`);
	await page.screenshot({ path: resolve(screenshotDir, '04-mcp-after-get-access.png'), fullPage: true });
} catch (err) {
